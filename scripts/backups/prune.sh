...
if docker exec restic-backup /bin/sh -c "prune"; then
    ...
    docker exec restic-backup restic -r /repos/local/restic forget --prune $PRUNE_ARGS
    if [[ "${CLOUD_RESTIC_REPOSITORY:-}" =~ ^rclone: ]]; then
        export RESTIC_FROM_PASSWORD="${RESTIC_PASSWORD:-}"
        docker exec -e RESTIC_FROM_PASSWORD="$RESTIC_FROM_PASSWORD" restic-backup restic -r "$CLOUD_RESTIC_REPOSITORY" forget --prune $PRUNE_ARGS
    fi
fi
